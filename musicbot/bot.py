    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.members and author_perms.skip_when_absent:
                newmsg = 'Skipping next song in `%s`: `%s` เพิ่มโดย`%s` as queuer not in voice' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
                player.skip()
            elif self.config.now_playing_mentions:
                newmsg = '%s - your song `%s` is กำลังเล่นใน `%s`!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = 'กำลังเล่นใน `%s`: `%s` เพิ่มโดย`%s`' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
        else:
...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='LouisFrancois-Bots/MusicBot ({})'.format(BOTVERSION), icon_url='https://i.imgur.com/gFHBoZA.png')
        e.set_author(name=self.user.name, url='https://github.com/LouisFrancois-Bots/MusicBot', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        async with self.aiolocks[_func_() + ':' + str(author.id)]:
            ...
            if 'entries' in info:
                ...
                reply_text = self.str.get('cmd-play-playlist-reply', "เพิ่มเพลง **%s** ไว้ในคิวแล้ว. อยู่คิวที่: %s")
            ...
            # If it's an entry
                ...
                reply_text = self.str.get('cmd-play-song-reply', "เพิ่มเพลง `%s` ไว้ในคิวแล้ว. อยู่คิวที่: %s")
                btext = entry.title
    ...
    async def cmd_summon(self, channel, guild, author, voice_channel):
        ...
        return Response(self.str.get('cmd-summon-reply', 'กำลังอยู่ใน `{0.name}`').format(author.voice.channel))
    ...
    async def cmd_restart(self, channel):
        ...
        and restarted.
        """
        await self.safe_send_message(channel, "\N{WAVING HAND SIGN} กำลังเริ่มต้นใหม่ ถ้าคุณอัพเดตบอท "
            "")
